...
export default function ShowFormModal({
  ...
  async function saveAllBatch() {
    ...
    setBatchResults([]);
    setFetchState("idle");
    toast.success(`已保存 ${batchResults.length} 条视频档案`);
  }
  ...
  const validate = () => {
    ...
    if (!title.trim()) e.title = "请输入视频标题（可先填写链接后点击「自动抓取」）";
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1.5 flex items-center">
                  视频标题 <span className="text-red-400 ml-0.5">*</span>
                  {fieldBadge("title")}
                </label>
                <input
                  type="text"
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  ...
                  {editingItem ? "保存修改" : batchMode ? `保存全部 (${batchResults.length})` : "添加视频"}
                </button>
              </div>
            </form>
          </motion.div>
