...
export default function BatchImportModal({ open, onClose, onSave }: BatchImportModalProps) {
  ...
  const parseMelon = async () => {
    ...
    if (!artistId) {
      setError("无法从链接中提取 artistId，请检查链接格式");
      return;
    }
    setLoading(true);
    setError("");
  ...
  const parseManual = () => {
    ...
    lines.forEach((line, i) => {
      ...
      if (!title) return;
      const roles = (rolesStr || "演唱")
        .split(/[,，]/)
        .map((r) => r.trim())
        .filter((r) => r) as MusicRole[];
      items.push({
        id: `m-batch-${Date.now()}-${i}`,
        title,
        album: album || "未知专辑",
        releaseDate: date || "",
        type: (type as MusicType) || "团体",
        roles: roles.length > 0 ? roles : ["演唱"],
        plays: "",
        link: link || "",
        isSelfComposed: roles.includes("作曲") || roles.includes("作词"),
      });
    });
    setPreview(items);
    setSelectedIds(new Set(items.map((item) => item.id)));
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="https://www.melon.com/artist/song.htm?artistId=672289"
                    className="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50/50 text-sm outline-none focus:border-sky-400 focus:ring-2 focus:ring-sky-100 transition-all"
                  />
                  <button
                    onClick={parseMelon}
                    disabled={loading || !url.trim()}
                    className="px-4 py-2.5 rounded-xl bg-sky-400 text-white text-sm font-medium hover:bg-sky-500 transition-colors disabled:opacity-50 flex items-center gap-1.5"
                  >
            ...
            <div className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  placeholder={`歌名 | 专辑 | 发行日期 | 类型 | 角色 | 链接
Missing You | Brother Act. | 2017-10-16 | 团体 | 演唱,作曲,作词 | https://music.apple.com
The Girl | Walk and Talk | 2024-02-29 | SOLO | 演唱,作曲,作词,编曲 | `}
                  rows={8}
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-sm font-bold text-gray-900">
                  预览 ({selectedIds.size}/{preview.length} 条已选择)
                </h3>
                <button
                  onClick={() => setSelectedIds(new Set(preview.map((i) => i.id)))}
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-gray-600">歌名</th>
                      <th className="px-3 py-2 text-left font-medium text-gray-600">专辑</th>
                    ...
                    {preview.map((item) => (
                      <tr
                        ...
                        className={`cursor-pointer transition-colors ${
                          selectedIds.has(item.id) ? "bg-sky-50/50" : "hover:bg-gray-50"
                        }`}
                      >
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
          ...
          >
            确认添加 {selectedIds.size > 0 ? `(${selectedIds.size}条)` : ""}
          </button>
        </div>
      </div>
    </div>
