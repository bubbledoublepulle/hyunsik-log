...
export default function BatchImportModal({ open, onClose, onSave }: BatchImportModalProps) {
  ...
  const parsePastedText = () => {
    ...
    const items: MusicItem[] = unique.map((s, idx) => ({ id: `m-batch-${Date.now()}-${idx}`, title: s.title, artist: s.artist || '', album: s.album || '未知专辑', albumNo: idx + 1, isTitleTrack: idx === 0, releaseDate: '', type: '录音室', roles: ['演唱'], plays: '', link: '', isSelfComposed: false }));
    setPreview(items);
  ...
  const parseMelon = async () => {
    ...
    try {
      ...
      for (let i = 0; i < titleMatches.length - 1; i += 2) {
        ...
        if (title && title !== "곡명") songs.push({ title, album: album || "未知专辑", date: date ? date.replace(/\./g, "-") : "", type: "录音室", roles: ["演唱"], plays: "", link: "", isSelfComposed: false, albumNo: 1, isTitleTrack: false });
      }
      const seen = new Set();
      const unique = songs.filter(s => { const key = s.title + "|" + s.album; if (seen.has(key)) return false; seen.add(key); return true; });
      if (unique.length === 0) { setError("未解析到歌曲"); return; }
      const items: MusicItem[] = unique.map((s, i) => ({ id: `m-batch-${Date.now()}-${i}`, title: s.title, artist: '', album: s.album, albumNo: i + 1, isTitleTrack: i === 0, releaseDate: s.date, type: s.type, roles: s.roles, plays: s.plays, link: s.link, isSelfComposed: s.isSelfComposed }));
      setPreview(items);
      setSelectedIds(new Set(items.map(i => i.id)));
  ...
  const parseManual = () => {
    ...
    lines.forEach((line, i) => {
      ...
      items.push({ id: `m-batch-${Date.now()}-${i}`, title, artist: artist || '', album: album || "未知专辑", albumNo: 1, isTitleTrack: false, releaseDate: date || "", type: (type as MusicType) || "录音室", roles: roles.length > 0 ? roles : ["演唱"], plays: "", link: link || "", isSelfComposed: roles.includes("作曲") || roles.includes("作词") });
    });
    setPreview(items);
    setSelectedIds(new Set(items.map(i => i.id)));
    setError("");
